...
stats_columns = ['Matches Played', 'Starts', 'Minutes', 'Non-Penalty Goals', 'Penalty Goals',
                 ...
                 'PPM_plt', 'onG_plt', 'onGA_plt', 'onxG_plt', 'onxGA_plt', 'fls', 'fld_ms', 'off_ms', 'Crs_ms', 'OG', 'recov', 'Won', 'Lost', 'Won%'
]

for stat in stats_columns:
    if df[stat].dtype != 'object':
        print(f"\nTop 3 cầu thủ có {stat} cao nhất:")
        print(df[['Name', stat]].nlargest(3, stat))  # Tìm 3 giá trị cao nhất

        print(f"\nTop 3 cầu thủ có {stat} thấp nhất:")
        print(df[['Name', stat]].nsmallest(3, stat))  # Tìm 3 giá trị thấp nhất
